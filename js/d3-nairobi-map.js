...
// Edited to suit the Nairobi data

/*===================================================
  Key changes:
  - Skip error-throwing for valid corners, 
    since the data already handles layout manually. A more 
    flexible version of populateLineDirections that assumes schematic 
    correctness and avoids errors due to “discontinuity”.
    The downside is that the corners are not smooth, but was 
    necessary to avoid too many errors
  - function drawLines() edited to add rail logic rendering rail lines
    differently to the BRT lines
  - 
...
(function (global, factory) {
    ...
    function populateLineDirections(line) {
      for (let nNode = 1; nNode < line.nodes.length; nNode++) {
        ...
        if (areParallel(prevVector, diff)) {
          ...
          let cornerPossible =
            (Math.abs(xDiff) == 1 && Math.abs(yDiff) == 1) || //Diagonal
            (Math.abs(xDiff) == 1 && Math.abs(yDiff) == 2) || //horizontal+vertical
            (Math.abs(xDiff) == 2 && Math.abs(yDiff) == 1) || //horizontal+vertical
            (Math.abs(xDiff) == 1 && Math.abs(yDiff) == 0) ||  // Additional - Purely horizontal
            (Math.abs(xDiff) == 0 && Math.abs(yDiff) == 1);    // Additional - Purely vertical
    ...
    //ORIGINAL ----- Replacement end here ------------------*/ 


    /*
    // ------ Replacement 1 ----------------
    // Try compass logic first, but if it fails, fallback to original behavior (e.g., allow (2, 1) if a corner is feasible)
    //This is because keeping a strict corner direction isnt working for the example data. MAy need to reformat my json
    function populateLineDirections(line) {
      function gcd(a, b) {
      ...
      for (let nNode = 1; nNode < line.nodes.length; nNode++) {
        ...
        let dx = Math.round(currNode.coords[0]) - Math.round(prevNode.coords[0]);
        let dy = Math.round(currNode.coords[1]) - Math.round(prevNode.coords[1]);
    
        if (dx === 0 && dy === 0) {
          throw new Error(`Repeated coordinates ${currNode.coords}`);
        }
    
        let factor = gcd(Math.abs(dx), Math.abs(dy)) || 1;
        let diff = [dx / factor, dy / factor];
    
        try {
          currNode.dir = compassBearing(diff);
        } catch (e) {
          // Fallback to original logic for legacy data
          let [xDiff, yDiff] = [dx, dy];
    
          if (nNode === 1) {
            prevNode.dir = compassBearing(diff);  // default to normalized direction
          }
    
          let prevVector = directionVector(prevNode.dir);
          if (areParallel(prevVector, [xDiff, yDiff])) {
            currNode.dir = compassBearing(prevVector);
          } else {
            let cornerPossible =
            ...
            if (cornerPossible) {
              let nextVector = apply2d((i) => xDiff - prevVector[i]);
              currNode.dir = compassBearing(nextVector);
            } else {
              throw new Error(
                `Invalid direction between ${prevNode.coords} and ${currNode.coords}: ${e.message}`
              );
            }
          }
        }
    
        if (nNode === 1 && !prevNode.dir) {
          prevNode.dir = currNode.dir;
        }
      }
    }
    
    // ------------------Replacement 1 ends here-----------------  
    */ 

     
    //---------------Replacement 2-----------------------
        /*We can simplify the function to skip error-throwing for valid corners, 
    since your data already handles layout manually. Here's a safer, more 
    flexible version of populateLineDirections that assumes schematic 
    correctness and avoids errors due to “discontinuity”:*/
    ...
    function populateLineDirections(line) {
      for (let i = 1; i < line.nodes.length; i++) {
        ...
        const dx = Math.round(curr.coords[0] - prev.coords[0]);
        const dy = Math.round(curr.coords[1] - prev.coords[1]);
    
        // Skip duplicate coordinates
        if (dx === 0 && dy === 0) {
          console.warn(`Repeated coords at index ${i}: ${curr.coords}`);
          continue;
        }
        ...
        try {
          ...
          // Set direction for prev and curr if not already set
          if (!prev.dir) prev.dir = bearing;
          if (!curr.dir) curr.dir = bearing;
        } catch (e) {
          console.warn(`Skipping unknown direction for coords [${prev.coords}] → [${curr.coords}]`);
        }
      }
    }
    
  // -------------------- Replacement 2 ends here ----------------------------*/
...
/*
  //-------------ORIGINAL------------------
    function line(data, xScale, yScale, lineWidth, lineWidthTickRatio) {
      ...
      for (let nNode = 1; nNode < data.nodes.length; nNode++) {
        ...
        let nextPoint = transform(
          nextNode.coords,
          nextVectorNorm,
          nNode === data.nodes.length - 0 ? endCorrection : 0 //----- changed -1 to -0
        );
  
         //----- Removing corners to see if my code works----
        ...
        //Replacement for my data which has sharp corners
        path += svgLineTo(nextPoint);
    ...
    //-------------Replacement 3-------------------
    // Overwrite the existing `line()` and remove the need for populateLineDirections()
    function line(data, xScale, yScale, lineWidth, lineWidthTickRatio) {
      let path = '';
      ...
      const shiftCoords = [
        (data.shiftCoords?.[0] || 0) * lineWidth / unitLength,
        (data.shiftCoords?.[1] || 0) * lineWidth / unitLength
      ];

      // Compute coordinate transformation
      function pointTransform(coords) {
        return [
          xScale(coords[0] + shiftCoords[0]),
          yScale(coords[1] + shiftCoords[1])
        ];
      }

      // Helper to compute cubic control points for gentle curves
      function computeControlPoints(xDiff, yDiff, start, end, lastType) {
        const dx = end[0] - start[0];
        const dy = end[1] - start[1];

        if (Math.abs(xDiff) === 1 && Math.abs(yDiff) === 1) {
          // Diagonal turn, use quadratic-like curve
          return {
            type: 'quadratic',
          ...
          // Schematic curve with longer arm
          const mid = [
            (start[0] + end[0]) / 2,
            (start[1] + end[1]) / 2
          ];
      ...
      for (let i = 0; i < nodes.length; i++) {
        const curr = nodes[i];
        const currPt = pointTransform(curr.coords);

        if (i === 0) {
          path += `M${currPt[0]},${currPt[1]}`;
        } else {
          const prev = nodes[i - 1];
          const prevPt = pointTransform(prev.coords);
          const xDiff = curr.coords[0] - prev.coords[0];
          const yDiff = curr.coords[1] - prev.coords[1];

          const control = computeControlPoints(xDiff, yDiff, prevPt, currPt);
          if (control?.type === 'quadratic') {
            path += `Q${control.control[0]},${control.control[1]},${currPt[0]},${currPt[1]}`;
          } else if (control?.type === 'cubic') {
            path += `C${control.controls[0][0]},${control.controls[0][1]},${control.controls[1][0]},${control.controls[1][1]},${currPt[0]},${currPt[1]}`;
          } else {
            path += `L${currPt[0]},${currPt[1]}`;
          }
        }
      }
    ...
    function map () {
      ...
      var lineWidth;
      var lineWidthMultiplier = 1.45;
      var lineWidthTickRatio = 3/3;
      var svg;
      var _data;
      var gMap;
      ...
      function drawLines() {
        gMap
          ...
          /*------------------------original-----Relacing draw lines to add rail logic
      function drawLines() {
        gMap
          .append('g')
          .attr('class', 'lines')
          ...
          .append('path')
          .each(function(d) {
            console.log('Drawing line:', d.name, 'mode:', d.mode);
          })
          .attr('d', function (d) {
            return line(d, xScale, yScale, lineWidth, lineWidthTickRatio);
          })
          ...
          .attr('stroke-dasharray', function (d) {
            return d.mode === 'rail' ? '4,2' : null;  // ← Add this line
          })
          .classed('line', true);
      }
         -------------end of original */
      ...
      //------------Replacement
      function drawLines() {
        const linesGroup = gMap.append('g').attr('class', 'lines');
        ...
        // Overlay dashed white line for rail mode
        lineSelection
          .filter((d) => d.mode === 'rail')
          .append('path')
          .attr('d', function (d) {
            return line(d, xScale, yScale, lineWidth, lineWidthTickRatio);
          })
      ...
      function drawLabels() {
        gMap
          ...
          .attr('fill', '#10137E') //'#10137E' <---original color
          .attr('dy', 0)
          .attr('x', function (d) {
            let shiftX =
              d.labelShiftX +
      ...
      function extractLines(data) {
        ...
        data.forEach(function (line) {
          var lineObj = {
            ...
            mode: line.mode || null,  // ← ✅ Add this line
            nodes: line.nodes,
            highlighted: false,
          };
